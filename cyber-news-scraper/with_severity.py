...
# Assign severity levels based on a custom scoring algorithm
# Example: severity is calculated as Threat_Level * (description_length / 100)
df['Severity_Score'] = df['Threat_Level'] * (df['description_length'] / 100)
...
# Convert the 'Date' column to a local string format
df['Date'] = df['Date'].dt.strftime('%Y-%m-%d %H:%M:%S')  # Convert datetime to string
...
X = df[['description_length', 'description_length_normalized', 'description_length_standardized', 'Hour', 'Minute', 'Threat_Level', 'Severity_Score']]
...
df.to_json(r"D:\vscode\SIH\cleaned_cybersecurity_data2.json", orient='records', lines=True)

print("Data processing completed. Cleaned data saved.")
